...
func acceptWebSocket(w http.ResponseWriter, r *http.Request) (net.Conn, *bufio.ReadWriter, error) {
	if r.Method != http.MethodGet {
		return nil, nil, badRequestError("websocket 仅支持 GET")
	}
	if !headerContainsToken(r.Header, "Connection", "upgrade") ||
		!strings.EqualFold(r.Header.Get("Upgrade"), "websocket") {
		return nil, nil, badRequestError("缺少 websocket upgrade 请求头")
	}
	if r.Header.Get("Sec-WebSocket-Version") != "13" {
		return nil, nil, badRequestError("不支持的 websocket 版本")
	}

	key := strings.TrimSpace(r.Header.Get("Sec-WebSocket-Key"))
	if key == "" {
		return nil, nil, badRequestError("缺少 Sec-WebSocket-Key")
	}
	if decoded, err := base64.StdEncoding.DecodeString(key); err != nil || len(decoded) != 16 {
		return nil, nil, badRequestError("无效的 Sec-WebSocket-Key")
	}
